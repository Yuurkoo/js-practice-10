...
function customAssign(...objects) {
  ...
  const arrAssign = Object.assign({}, ...objects);

  for (const key in arrAssign) {
    if (typeof arrAssign[key] === "number") {
      arrAssign[key] = arrAssign[key].toString();
    }
  }
...
function customObjectFromEntries(entries) {
  ...
  const sortArr = entries.map((point) => {
...
function checkProperty(obj, prop) {
  ...
  const checkValue = obj.hasOwnProperty(prop);
  return checkValue;
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const addAssignment = Object.assign(obj1, obj2);
  const isObgFreeze = Object.freeze(addAssignment);

  // if (Object.isFrozen(isObgFreeze)) {
  //   return "заморожений";
  // } else {
  //   return "error";
  // }
  return addAssignment; // так до виведе "заморожений", але завдання вимагає вивести результат - обєднаний об'єкт
}
...
function sealAndCheck(obj) {
  ...
  const sealArr = Object.seal(obj);
  if (Object.isSealed(sealArr)) {
    return true;
  }
}
...
function checkOwnership(obj1, obj2, key) {
  ...
  const cheackArr1 = obj1.hasOwnProperty(key);
  const cheackArr2 = obj2.hasOwnProperty(key);

  if (cheackArr1 === cheackArr2) {
    return true;
  } else if (cheackArr1 === true && cheackArr2 === true) {
    return `Значення cheackArr1 = ${cheackArr1} , Значення cheackArr2 = ${cheackArr2}`;
  } else {
    return false;
  }
}
...
function getObjectValuesSum(obj) {
  ...
  const allArrValues = Object.values(obj);

  const sum = allArrValues.reduce(
